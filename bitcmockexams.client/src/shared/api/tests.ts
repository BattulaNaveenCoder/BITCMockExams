...
export const useTestsApi = () => {
  ...
  return useMemo(() => {
    ...
    const getAllExamQuestionsById = async (
      testId: string,
      showGlobalLoader: boolean = true
    ): Promise<any[] | null> => {
      const base = isDev ? `${window.location.origin}/a2z-tests` : 'https://a2z-tests.azurewebsites.net';
      const endpoint = `${base}/api/TestQuestion/GetAllExamQuestionsbyId/${encodeURIComponent(testId)}`;
      try {
        const data = await api.get(endpoint, { showGlobalLoader });
        const payload = (data as any)?.data ?? data;
        ...
        console.error('Failed to fetch exam questions by TestId:', { testId, error });
        return null;
      }
    };

    const getAllOptionsWithQuestionId = async (
      questionId: string,
      showGlobalLoader: boolean = true
    ): Promise<any[] | null> => {
      const base = isDev ? `${window.location.origin}/a2z-tests` : 'https://a2z-tests.azurewebsites.net';
      const endpoint = `${base}/api/TestQuestion/GetAllOptionsWithQuestionId/${encodeURIComponent(questionId)}`;
      try {
        const data = await api.get(endpoint, { showGlobalLoader });
        const payload = (data as any)?.data ?? data;
        if (Array.isArray(payload)) return payload as any[];
        ...
        console.error('Failed to fetch options for question:', { questionId, error });
        return null;
      }
    };
    ...
    return {
      ...
      getBuyerTestById,
      getAllExamQuestionsById,
      getAllOptionsWithQuestionId,
    };
  }, [api]);
};
